...
def main(radius, thickness, out_path):
    ...
    hit = (bi >= 0)[flat]
    P = P_all[flat]; N = np.zeros_like(P)
    N[hit] = box_normal(P[hit], bi[flat][hit])

    ao = np.full(len(flat), np.nan)
    idx = np.flatnonzero(hit); rng = np.random.default_rng(99)
    CHUNK = 1500
    for s in range(0, len(idx), CHUNK):
        sel = idx[s:s + CHUNK]
        p = P[sel] + N[sel] * EPS; n = N[sel]; tg, bt = onb(n); m = len(sel)
        u1 = rng.random((m, SAMPLES)); u2 = rng.random((m, SAMPLES))
        rr = np.sqrt(u1); th = 2 * np.pi * u2
        dirs = (tg[:, None, :] * (rr * np.cos(th))[:, :, None]
                + bt[:, None, :] * (rr * np.sin(th))[:, :, None]
                + n[:, None, :] * np.sqrt(np.maximum(1 - u1, 0))[:, :, None]).reshape(-1, 3)
        orig = np.repeat(p, SAMPLES, axis=0)
        rad = np.full(len(orig), radius)
        if _os.environ.get("AO_DIST_RADIUS"):
            zc = (P[sel] - CAM_POS) @ F
            rad = np.repeat(float(_os.environ["AO_DIST_RADIUS"]) * 2.0 * TAN * zc, SAMPLES)
        blocked = np.zeros(len(orig), bool)
        for k in range(1, STEPS + 1):
            q = orig + dirs * (rad * (k / STEPS))[:, None]
            px, py, qz = to_screen(q)
            ix = np.clip(px.astype(np.int32), 0, W - 1); iy = np.clip(py.astype(np.int32), 0, H - 1)
            surf = zbuf[iy, ix]
    ...
    np.savez_compressed(out_path, ao=ao.reshape(len(ys), len(xs)), hit=hit.reshape(len(ys), len(xs)),
                        stride=STRIDE, w=W, h=H, radius=radius)
    v = ao[hit]
    print("screen-space reference: radius %.2f thickness %.2f  AO mean %.4f min %.4f" % (radius, thickness, v.mean(), v.min()))
